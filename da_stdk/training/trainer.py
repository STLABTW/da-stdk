...
from da_stdk.utils import ModelEMA
...
def evaluate_model(
    ...
    if config is not None and config.get("regression_type") == "multi-quantile":
        ...
        metrics["picp_90"] = float(compute_picp(preds, trues, quantile_levels, alpha=0.1))
...
class Trainer:
    ...
    def __init__(
        ...
        self.warmup_steps = warmup_epochs * len(train_loader) if warmup_epochs > 0 else 0
        ...
        self.quantile_levels = config.get("quantile_levels", [0.05, 0.25, 0.5, 0.75, 0.95])
    ...
    def fit(self) -> Tuple[nn.Module, Dict[str, List], List[Tuple[int, np.ndarray]]]:
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                    ...
                    if use_delta_reparam:
                        ...
                        if non_crossing_lambda > 0:
                            ...
                            if delta_params is not None:
                                l1_penalization = config.get("non_crossing_l1_penalization", False)
                                if l1_penalization:
                ...
                if config.get("spatial_learnable", False):
                    ...
                    if domain_penalty_weight > 0:
                        loss = loss + domain_penalty_weight * model.compute_domain_penalty()
                    movement_penalty_weight = config.get("movement_penalty_weight", 0.0)
                    if movement_penalty_weight > 0:
                        loss = loss + movement_penalty_weight * model.compute_movement_penalty()
                ...
                if config.get("grad_clip", 0) > 0:
                    if config.get("spatial_learnable", False):
                        ...
                        torch.nn.utils.clip_grad_norm_(basis_params, config["grad_clip"] * 0.1)
                ...
                if has_validation:
                    for batch in val_loader:
                        ...
                        if regression_type == "mean":
                            ...
                            for q_idx, q_level in enumerate(quantile_levels):
                                losses.append(
                                    quantile_loss(y_pred[:, q_idx : q_idx + 1], y, q_level)
                                )
                            loss = torch.mean(torch.stack(losses))
                            use_delta_reparam = config.get("use_delta_reparameterization", False)
                            if use_delta_reparam and config.get("non_crossing_lambda", 0.0) > 0:
                                delta_params = model.get_delta_parameters()
                                if delta_params is not None:
                                    if config.get("non_crossing_l1_penalization", False):
            ...
            if has_validation and len(val_preds) > 0:
                ...
                val_rmse = float(np.sqrt(np.mean((val_preds_for_rmse - val_trues_c) ** 2)))
                val_check_loss = None
